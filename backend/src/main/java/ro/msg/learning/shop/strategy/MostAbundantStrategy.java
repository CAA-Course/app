...
public class MostAbundantStrategy implements StrategyInterface {
    ...
    public List<LocationStrategyModel> getLocations(OrderProductQuantityDTO order) {
        ...
        for (ProductAndQuantityDTO product : order.getProductAndQuantityDTOList()) {

            List<Location> locationsWithProduct = locationRepository.findAll()
                    .stream()
                    .filter(location -> hasLocationProduct(product, location))
            ...
            for (Location location: locationsWithProduct) {
                ...
                if(stockQuantity != null && maxQuantity < stockQuantity && stockQuantity >= product.getProductQuantity()) {

                        maxQuantity = stockQuantity;
                        Product orderProduct = productRepository.findById(product.getProductId()).orElse(null);

                        if(orderProduct != null){
                            mostAbundantLocation = new LocationStrategyModel(location, orderProduct, product.getProductQuantity());
                        }
                }
            }
        ...
        if (locations.isEmpty() || locations.size() != order.getProductAndQuantityDTOList().size())
    ...
    private boolean hasLocationProduct(ProductAndQuantityDTO product, Location location) {
        ...
        return stockProductsId.contains(product.getProductId());
    }
}
